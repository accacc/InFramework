...
namespace IF.Emarsys
{
    public class EmarsysApiClient
    {
        ...
        public ApiResponse SendEmail(List<Contact> contacts, int eventId, Encoding encoding = null)
        {
            RootObject @object = new RootObject();
            @object.key_id = 3;
            @object.contacts = contacts;
   
                ApiResponse apiResponse = this._manager.Send<ApiResponse>(RequestType.POST, String.Format("event/{0}/trigger", eventId), @object, encoding);
