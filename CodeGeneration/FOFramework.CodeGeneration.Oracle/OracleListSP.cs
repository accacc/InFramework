...
{
    public class OracleListSP : StoredProcedure,IGenerateCode
    {
        ...
        public CodeTemplate GenerateCode()
        {
            ...
            foreach (var parameter in this.Parameters)
            {
                if (parameter.Type == typeof(String))
                {
                    ...
                    builder.AppendLine(String.Format("P_{0} IS NULL OR {1}.{2} like {2}||'%') ", parameter.Name,parameter.Name,parameter.TableName,parameter.Name));
                }
                else
                {
