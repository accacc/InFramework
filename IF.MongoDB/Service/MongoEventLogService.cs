...
namespace IF.MongoDB.Service
{
    public class MongoEventLogService : IEventLogService
    {
        ...
        public MongoEventLogService(string url, string db)
        {
        ...
        public async Task<PagedListResponse<EventLog>> GetPaginatedEventLogsAsync(DateTime BeginDate, DateTime EndDate, string ServiceName, string EventType, Guid EventId, Guid SourceId, int PageNumber = 0, int PageSize = 50)
        {
            ...
            var list = await _context.EventLogs.Find(filter).Project<EventLog>(fields).Skip((PageNumber - 1) * PageSize).Limit(PageSize).SortByDescending(s => s.CreationTime).ToListAsync();


            var count = await _context.EventLogs.CountDocumentsAsync(filter);
        ...
        public async Task SaveEventAsync(IFEvent @event, string serviceName, EventStateEnum eventState)
        {
            try
            {
                ...
                await _context.EventLogs.InsertOneAsync(eventLogEntry);
